...
import com.tawabsoft.taxi.rider.activities.coupon.adapters.CouponsRecyclerViewAdapter
import com.tawabsoft.taxi.rider.activities.coupon.adapters.CouponsRecyclerViewAdapter.OnCouponItemInteractionListener
import com.tawabsoft.taxi.rider.databinding.ActivityCouponBinding
import com.tawabsoft.taxi.rider.networking.socket.AddCoupon
import com.tawabsoft.taxi.rider.networking.socket.ApplyCoupon
import com.tawabsoft.taxi.rider.networking.socket.GetCoupons
...
class CouponActivity : BaseActivity() {
    ...
    fun applyCoupon(code: String) {
        ApplyCoupon(code).execute<Double> {
            when(it) {
                is RemoteResponse.Success -> {
                    val intent = Intent()
                    intent.putExtra("coupon", coupon)
                    intent.putExtra("costAfterCoupon", it.body)
