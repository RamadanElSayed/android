...
import com.tawabsoft.taxi.rider.databinding.ItemServiceBinding
...
class ServicesListAdapter(private val services: List<Service>,
    ...
    class ViewHolder(var binding: ItemServiceBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        private fun updatePrice(service: Service, distance: Int, duration: Int, currency: String) {
            ...
            if (service.distanceFeeMode == Service.DistanceFee.PickupToDestination) {
                cost += (distance.toDouble() / 100 * service.perHundredMeters) + (duration.toDouble() / 60 * service.perMinuteDrive)
            }
            if(cost < (service.minimumFee ?: 0.0)) {
                cost = service.minimumFee ?: 0.0
            }
